...
try:
    ...
    for drone_config in drones:
        ...
        if serial_mavlink:
            ...
            mavlink_source = f"/dev/{Radio_Serial_Port}:{Radio_Serial_Baudrate}"
        ...
        #Append QGC Connection
        endpoints.append(f"-e {drone_config['gcs_ip']}:14550")

        #Append Connection for GCS Script
        endpoints.append(f"-e 127.0.0.1:{drone_config['debug_port']}")
